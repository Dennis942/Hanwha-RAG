...
function documentValuesByScope(document: any, scope: string) {
  if (scope === "title") {
...
export async function POST(request: Request) {
  ...
  if (documentError) {
    return apiError(documentError.message, {
      status: 500,
      code: "documents_search_load_failed",
      step: "documents-search",
  ...
  if (chunkError) {
    return apiError(chunkError.message, {
      status: 500,
      code: "chunks_search_load_failed",
      step: "chunks-search",
      details: chunkError
    });
  }
  ...
  const results = (documents ?? [])
    ...
    .map((document: any) => {
      ...
      const documentMatches = includesKeyword(
        documentValuesByScope(document, scope),
        query
      );
      const shouldSearchChunks = scope === "all" || scope === "content";
      ...
      return {
        ...
        indexing_required: document.status !== "indexed",
        created_at: document.created_at,
        description: document.description,
        matched_chunks: shouldSearchChunks ? matchedChunks : [],
