...
export async function DELETE(request: Request) {
  ...
  if (!id) {
    return apiError("삭제할 질문 이력 ID가 필요합니다.", {
      status: 400,
      code: "chat_log_id_required",
      step: "validation"
    });
  }
  ...
  if (error) {
    return apiError(error.message, {
      status: 500,
      code: "chat_log_delete_failed",
      step: "chat-history-delete",
      details: error
    });
  }

  return apiOk({ message: "질문 이력이 삭제되었습니다." });
}
